...
class ResNet_features(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        # self.layer1 = self._make_layer(block=block, planes=256, num_blocks=self.layers[0])
        # self.layer2 = self._make_layer(block=block, planes=512, num_blocks=self.layers[1], stride=2)
        # self.layer3 = self._make_layer(block=block, planes=1024, num_blocks=self.layers[2], stride=2)
        # self.layer4 = self._make_layer(block=block, planes=2048, num_blocks=self.layers[3], stride=2)
...
class ResNet_features_new(nn.Module):
    '''
    the convolutional layers of ResNet
    the average pooling and final fully convolutional layer is removed
    '''
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        super(ResNet_features_new, self).__init__()
...
def resnet50_features_new(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features_new(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        my_dict = model_zoo.load_url(model_urls['resnet50'], model_dir=model_dir)
        my_dict.pop('fc.weight')
        my_dict.pop('fc.bias')
