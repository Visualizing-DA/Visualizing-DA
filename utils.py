...
def get_dataset(dataset_name, root, source, target, train_source_transform, val_transform, train_target_transform=None):
    ...
    if dataset_name == "Digits":
        ...
        val_dataset = concat_dataset(root=root, tasks=target, download=True, transform=val_transform,
                                     start_idx=len(source))
        if dataset_name == 'DomainNet' or dataset_name == 'DomainNet126':
...
def get_train_push_test(dataset_name, root, source, target, train_transform, push_transform, test_transform):
    ...
    if dataset_name == "Digits":
        ...
        train_target_dataset = datasets.__dict__[target[0]](osp.join(root, target[0]), download=True,
                                                            transform=train_transform)
        push_dataset = datasets.__dict__[target[0]](osp.join(root, target[0]), split='test',
                                                                  download=True, transform=push_transform)
        class_names = datasets.MNIST.get_classes()
        num_classes = len(class_names)
    elif dataset_name in datasets.__dict__:
        # load datasets from tllib.vision.datasets
        ...
        train_source_dataset = concat_dataset(root=root, tasks=source, download=True, transform=train_transform,
                                              start_idx=0)

        push_dataset = concat_dataset(root=root, tasks=source, download=True, transform=push_transform,
                                     start_idx=0)

        train_target_dataset = concat_dataset(root=root, tasks=target, download=True, transform=train_transform,
                                              start_idx=len(source))

        test_dataset = concat_dataset(root=root, tasks=target, download=True, transform=test_transform,
                                              start_idx=len(source))

        if dataset_name == 'DomainNet' or dataset_name == 'DomainNet126':
            test_dataset = concat_dataset(root=root, tasks=target, split='test', download=True, transform=test_transform,
                                          start_idx=len(source))
        else:
        ...
        raise NotImplementedError(dataset_name)
    return train_source_dataset, train_target_dataset, push_dataset, test_dataset, num_classes, class_names




def get_train_push_test_lw(dataset_name, root, source, target, train_transform, push_transform, test_transform):
